    ...
    <div class="mws-panel-body no-padding">
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all">
        		...
        		<tr class="odd" style="text-align: center;">
                    ...
                    <td>{{ $v->surl }}</td>
                    <td>
                        <img src="/upload/{{ $v->simg }}"  style="width: 100px;height: 60px;margin:auto" class="thumbnail">
                    </td>
                    <td>@if($v->sstatu == '1') 显示  
                    	@elseif($v->sstatu == '2') 不显示 
                    	@endif</td>
                    <td>{{ $v->created_at }}</td>
                    <td>{{ $v->updated_at }}</td>
                    <td>
                        <form action="/admin/slides/{{ $v->id }}" method="post" style="display: inline-block;">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
