                ...
                @foreach ($data as $k=>$v)
                    <tr class="odd">
                        ...
                        <td>
                            ...
                            @if(session('id') == $v->id)
                            <form action="/admin/users/{{ $v->id }}" method="post" style="display: inline-block;display: none;">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <input type="submit" onclick="return confirm('您确认要删除吗?')" value="删除" class="btn btn-danger">
                            </form> 
                            @else
