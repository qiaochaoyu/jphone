...
$.initProv = function(prov, city, defaultProv, defaultCity) {
    ...
    provHtml += '<option value="0">请选择</option>';
    for(var i = 0; i < $._cityInfo.length; i++) {
        provHtml += '<option value="' + (i+1) + '"' + ((hasDefaultProv && (i+1) == defaultProv) ? ' selected="selected"' : '') + '>' + $._cityInfo[i].n + '</option>';
    }
...
$.initCities = function(provEl, cityEl, defaultCity) {
    ...
    if(provEl.val() != '' && parseInt(provEl.val()) > 0) {
        var cities = $._cityInfo[parseInt(provEl.val())-1].c;
        var cityHtml = '';

        cityHtml += '<option value="0">请选择</option>';
        for(var i = 0; i < cities.length; i++) {
            cityHtml += '<option value="' + (i+1) + '"' + ((hasDefaultCity && (i+1) == defaultCity) ? ' selected="selected"' : '') + '>' + cities[i] + '</option>';
        }
        cityEl.html(cityHtml);
    } else {
        cityEl.html('<option value="0">请选择</option>');
    }
};